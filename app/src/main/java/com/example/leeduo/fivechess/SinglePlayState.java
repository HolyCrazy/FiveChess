...
public class SinglePlayState extends State {
    ...
    public SinglePlayState(Game game) {
        super(game);
        this.game = game;
    ...
    public void init() {
        ...
        recorder.clear();
        robot = new AIRobot(recorder,getLineToLine(),getChessFromX(),getChessFromY(),getFirstToLastLine());
        handler = new Handler(){
            @Override
                    ...
                    case 100:
                        if(winFlag == false){
                            if(turnToBlack == false){
                                robot.think();
                                bestPoint = robot.getBestPoint();
                                Log.d("---------", "handleMessage: "+bestPoint );
                                    recorder.put(bestPoint,false);
                                turnToBlack = true;
                                //判断是否有人胜利
